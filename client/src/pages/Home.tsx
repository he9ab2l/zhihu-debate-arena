...
export default function Home() {
  ...
  const runAiSummary = (scope: "evidence" | "debate" | "rounds", evidenceId?: string) => {
    if (!active) return;
    if (!auth.isAuthenticated) { toast.info("登录后才能使用 AI 总结"); startLogin(); return; }
  ...
  return (
    <div className={`app-shell ${sidebarOpen ? "sidebar-visible" : "sidebar-collapsed"}`}>
      ...
      <Sheet open={aiOpen} onOpenChange={setAiOpen}><SheetContent className="ai-sheet"><SheetHeader><SheetTitle><Sparkles size={18} />AI 辅助总结</SheetTitle></SheetHeader><div className="sheet-note"><ShieldCheck size={16} />只基于本场真实知乎检索摘要生成；摘要不是全文，不替你投票。</div><div className="ai-scope-tabs"><Button variant={aiScope === "evidence" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("evidence", active?.evidence?.[0]?.id)}>单篇证据</Button><Button variant={aiScope === "debate" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("debate")}>整场辩题</Button><Button variant={aiScope === "rounds" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("rounds")}>三回合综合</Button></div>{aiScope === "evidence" && active?.evidence?.length ? <div className="ai-evidence-picker">{active.evidence.map((item: any) => <button key={item.id} className={item.id === aiEvidenceId ? "is-active" : ""} onClick={() => runAiSummary("evidence", item.id)}>{item.title}</button>)}</div> : null}<div className="ai-result">{aiMutation.isPending ? <div className="loading-card"><Loader2 className="spin" />正在生成总结…</div> : aiMutation.data?.content ? <p>{aiMutation.data.content}</p> : <div className="empty-evidence"><Sparkles size={22} /><strong>选择一种总结范围</strong><p>AI 输出会与原始证据分开标注。</p></div>}</div></SheetContent></Sheet>
    </div>
  );
}
