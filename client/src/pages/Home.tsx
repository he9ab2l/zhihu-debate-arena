...
function SourceBadge({ source }: { source: "zhihu" }) {
  return <Badge className="source-badge source-badge-live"><span className="status-dot" />知乎站内检索</Badge>;
}
...
export default function Home() {
  ...
  return (
    <div className={`app-shell ${sidebarOpen ? "sidebar-visible" : "sidebar-collapsed"}`}>
      ...
      <Sheet open={aiOpen} onOpenChange={setAiOpen}><SheetContent className="ai-sheet"><SheetHeader><SheetTitle><Sparkles size={18} />AI 辅助总结</SheetTitle></SheetHeader><div className="sheet-note"><ShieldCheck size={16} />只基于本场真实知乎检索摘要生成；摘要不是全文，不替你投票。模型需要整理证据，通常需要约 30 秒。</div><div className="ai-scope-tabs"><Button variant={aiScope === "evidence" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("evidence", active?.evidence?.[0]?.id)}>单篇证据</Button><Button variant={aiScope === "debate" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("debate")}>整场辩题</Button><Button variant={aiScope === "rounds" ? "default" : "outline"} size="sm" onClick={() => runAiSummary("rounds")}>三回合综合</Button></div>{aiScope === "evidence" && active?.evidence?.length ? <div className="ai-evidence-picker">{active.evidence.map((item: any) => <button key={item.id} className={item.id === aiEvidenceId ? "is-active" : ""} onClick={() => runAiSummary("evidence", item.id)}>{item.title}</button>)}</div> : null}<div className="ai-result">{aiMutation.isPending ? <div className="loading-card"><Loader2 className="spin" />正在生成总结…<small>请保持面板打开，服务端会在 90 秒内返回。</small></div> : aiMutation.isError ? <div className="empty-evidence"><CircleHelp size={22} /><strong>这次请求没有完成</strong><p>{aiMutation.error.message}</p><Button variant="outline" size="sm" onClick={() => runAiSummary(aiScope, aiEvidenceId)}>重新生成</Button></div> : aiMutation.data?.content ? <Streamdown>{aiMutation.data.content}</Streamdown> : <div className="empty-evidence"><Sparkles size={22} /><strong>选择一种总结范围</strong><p>AI 输出会与原始证据分开标注。</p></div>}</div></SheetContent></Sheet>
    </div>
  );
}
...
function EmptyEvidence() {
  return <div className="empty-evidence"><BookOpen size={25} /><strong>还没有真实论据</strong><p>当前检索没有返回可核验来源，系统不会用虚构作者或赞同数填充这里。</p></div>;
}
